...
import { SliderNumberInput, SectionHeader } from './shared'
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3 pt-2 border-t border-gray-700">
        <label className="text-xs text-gray-400 font-medium">⚡ Hareket Ayarları</label>
        
        <SliderNumberInput
          label="Hareket Hızı"
          value={player.moveSpeed}
          onChange={(v) => updatePlayer({ moveSpeed: v })}
          min={1}
          max={15}
          step={0.5}
          unit="m/s"
        ...
        <SliderNumberInput
          label="Göz Yüksekliği"
          value={player.eyeHeight}
          onChange={(v) => updatePlayer({ eyeHeight: v })}
          min={0.5}
          max={2.5}
          step={0.05}
          unit="m"
        ...
        {['X', 'Y', 'Z'].map((axis, i) => (
          <div key={axis}>
            ...
            <input
              ...
              className={`w-full bg-editor-input border border-gray-600 rounded px-2 py-1.5 text-white text-sm focus:border-${
